...
const App = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3001/persons")
        .then(response =>
          setPersons(response.data.map(item => item))
        )
  }, []      
  )
  ...
  const handleSubmit = (event) => {
    ...
    axios.post("http://localhost:3001/persons", noteObject).then(response => setPersons([...persons, response.data])).catch(error => alert(error))
