  ...
  const handleSubmit = (event) => {
    ...
    if (persons.filter(item => item.name.toLowerCase().trim() === newName.toLowerCase().trim()).length > 0) {
      if(window.confirm(`${newName} is already added to phonebook. Do you want to replace it with the current details ?`)){
        ...
        service.updatePerson(id, personObject).then(newPerson => setPersons(persons.reduce((newValue, item) => newPerson.id === item.id ? [...newValue, newPerson] : [...newValue, item], []))).catch(error => alert("Could not create person: ", error))
      }
      return
    }
